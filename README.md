...
의안번호만 입력하면 열린국회정보 API에서 의안 정보를 수집하고, 표지부터 제안이유·주요내용까지 hwpx 템플릿에 자동으로 채워 다운로드하도록 해준다.
...
- 의안번호 직접 입력 또는 엑셀 파일(의안번호 목록) 업로드 (여러 건 동시 처리)
- 회기·위원회·담당자 옵션 입력
- 자동생성된 검토보고서(hwpx) + 의안원문(hwp) → ZIP 파일로 일괄 다운로드
- 단, 검토보고서 작성을 위해 필요한 신구조문대비표는 ZIP 파일에 포함된 의안원문에서 수동 복사·붙여넣기 필요
